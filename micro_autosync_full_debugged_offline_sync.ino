...
#define ban_date_0_addr 4
#define ban_date_1_addr 5
#define ban_mn_addr 6
#define ban_year_0_addr 7
#define ban_year_1_addr 8
#define ban_year_2_addr 9
#define ban_year_3_addr 10
...
void loop()                                                         
{
    ...
    if(count == 5)
    {
      ...
      //if(((abs(elapsed_time - current_time)) >= (60000 - (prev_sec)*1000)) || (trans_flag == 1) ) //((abs(34 - prev_sec))*1000))
      if(((abs(elapsed_time - current_time)) >= (60000 - (prev_sec)*1000)))    //////((abs(34 - prev_sec))*1000))
      {
        ...
        if((prev_min != myRTC.minutes) && (trans_flag == 1))
        {
          ...
          //elapsed_time = current_time - 3500;
          trans_flag = 0;
        }
    ...
    if(count == 4)
    {
      ...
      //reads from EERPROM in case wifi is not available
      if(wifi_timeout >= 15)
      {
        bn_date[0] = EEPROM.read(ban_date_0_addr);
        bn_date[1] = EEPROM.read(ban_date_1_addr);
        
        bn_m = EEPROM.read(ban_mn_addr);
        
        bn_year[0] = EEPROM.read(ban_year_0_addr);
        bn_year[1] = EEPROM.read(ban_year_1_addr);
        bn_year[2] = EEPROM.read(ban_year_2_addr);
        bn_year[3] = EEPROM.read(ban_year_3_addr);
      }
...
void set_ban_date(void)
{
  ...
  else if(rec_data.indexOf("+IPD,132") > 0)
  {
   ...
   // lcd.print("Setting Bangla Date...");
      ...
      //EEPROM Write
      EEPROM.write(ban_date_0_addr,bn_date[0]);
      EEPROM.write(ban_date_1_addr, bn_date[1]);
        
      EEPROM.write(ban_mn_addr, bn_m);
        
      EEPROM.write(ban_year_0_addr, bn_year[0]);
      EEPROM.write(ban_year_1_addr, bn_year[1]);
      EEPROM.write(ban_year_2_addr, bn_year[2]);
      EEPROM.write(ban_year_3_addr, bn_year[3]);
